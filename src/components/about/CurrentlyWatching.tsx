...
interface Show {
    ...
    progress: number;
    myrating: number | null; // <--- Added this field
}
...
// Default colors constant to reuse
...
export default function CurrentlyWatching() {
    ...
    // Replace simple 'colors' state with a Cache Map
    const [colorCache, setColorCache] = useState<Record<string, ExtractedColors>>({});

    useEffect(() => {
        const checkMobile = () => setIsMobile(window.innerWidth < 519);
        checkMobile();
    ...
    useEffect(() => {
        if (!isLoading) return;
        const interval = setInterval(() => setLoaderProgress(prev => (prev >= 100 ? 0 : prev + 2)), 50);
        return () => clearInterval(interval);
    }, [isLoading]);

    // Fetch shows AND trigger background color prefetching
    useEffect(() => {
    ...
    // Function to fetch and cache colors
    const prefetchColor = async (backdropUrl: string) => {
        if (colorCache[backdropUrl]) return;

        try {
            const response = await fetch(`/api/spotify/extract-color?imageUrl=${encodeURIComponent(backdropUrl)}`);
            if (response.ok) {
                const data = await response.json();
                setColorCache(prev => ({
                    ...prev,
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => setCurrentIndex(prev => (prev + 1) % shows.length), 20000);
        return () => clearInterval(interval);
    }, [shows.length]);
    ...
    if (isLoading || shows.length === 0) {
        return (
            <Squircle cornerRadius={isMobile ? 40 : 50} cornerSmoothing={0.7} className="w-full min-h-[350px] bg-zinc-900/50 p-6 text-white relative overflow-hidden">
    ...
    const activeColors = (currentShow.backdrop && colorCache[currentShow.backdrop]) 
    ...
    return (
        ...
        >
            ...
            {shows.length > 1 && (
               <>
                   <button onClick={goToPrevious} className="absolute left-2 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-black/20 hover:bg-black/50 border border-white/10 backdrop-blur-sm opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ease-out">
                       <ChevronLeft size={isMobile? 15: 20} className="text-white/70" />
                   </button>
                   <button onClick={goToNext} className="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-black/20 hover:bg-black/50 border border-white/10 backdrop-blur-sm opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ease-out">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center gap-16 py-8 px-8 md:px-10">
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        // Reduced gap from 16 to 12 here to account for the added height of stars
                        className="flex flex-col gap-12" 
                    >
                        {/* Wrapper for Logo + Stars to keep them tight together */}
                        <div className="flex flex-col gap-2 md:gap-4 justify-center">
                             <div className="h-16 md:h-20 flex items-center justify-start shrink-0 max-w-[75%]">
                                {currentShow.logo ? (
                                    <div className="relative w-40 md:w-48 h-full origin-left scale-100 md:scale-125">
                                        <Image src={currentShow.logo} alt={currentShow.title} fill className="object-contain object-left" unoptimized />
                                    </div>
                                ) : (
                            ...
                            {currentShow.myrating !== null && currentShow.myrating !== undefined && (
                                // 1. Added 'group/rating' and 'relative' here
                                <div className="group/rating relative flex items-center gap-1 cursor-default w-fit">
                                    
                                    {/* 2. THE TOOLTIP (Hidden by default, shows on hover) */}
                                    ...
                                    {/* 3. Your Existing Stars Loop */}
                                    {[...Array(10)].map((_, i) => (
                                        <div key={i} className="relative w-3 h-3 md:w-4 md:h-4">
                                            <Image 
                                                src={i < (currentShow.myrating as number) ? '/starfilled.png' : '/starblack.png'} 
                                                alt={i < (currentShow.myrating as number) ? 'filled star' : 'empty star'}
                                                fill
                        ...
                        <div className="flex items-center justify-between gap-6 w-full">
                            ...
                            {currentShow.provider_logo && (
                                <div className="relative scale-[1.75] md:scale-[3] md:mb-4 origin-right w-6 h-6 shrink-0 rounded-full overflow-hidden shadow-sm">
                                    <Image src={currentShow.provider_logo} alt="Provider" fill className="object-cover" unoptimized />
                                </div>
                            )}
                        </div>
                    </motion.div>
