...
export default function DuolingoWidget({ onHoverColor, onLeaveColor }: DuolingoWidgetProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const streakChars = [
            'duo1.svg', 'duo3.svg', 'duo4.svg', 'duo7.svg',
            'duo8.png', 'duo9.png', 'duo10.png', 'duo11.png',
            'duo12.gif', 'duo13.gif', 'duo14.gif'
          ];
  ...
  const handleMouseEnter = () => {
    if (data?.dailyGradient?.to) {
      // Use the 'to' color for vibrancy
      onHoverColor?.(data.dailyGradient.to, '#ffffff');
    } else {
      // Fallback to Duolingo Green
      onHoverColor?.('#58CC02', '#ffffff');
    }
  };
            ...
            <div className="flex flex-col items-center gap-0 shrink-0 -mt-2">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10">
                  <Image
                    src={
                      data.isLessonComplete
                        ? '/duolingo/fire-stroke.svg'
                        : '/duolingo/nofire.svg'
                    }
                    alt="Streak"
                    fill
                    className={`object-contain ${
                      data.isLessonComplete ? 'animate-none' : 'opacity-80'
                    }`}
                  />
                </div>
            ...
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col leading-none">
                  ...
                  <span className="text-white/60 font-medium text-xs opacity-80">
                    {data.username}
                  </span>
                </div>
              </div>
              ...
              <div className="w-1/2 h-full -ml-3 relative">
                ...
                >
                  {data.courses.map((course) => (
                    <div
                      key={course.lang}
                      className="flex items-center gap-3 group/lang cursor-default"
                    >
