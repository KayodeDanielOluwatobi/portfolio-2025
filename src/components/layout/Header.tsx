...
export default function Header({ currentBrand = 'default', onMobileMenuToggle }: HeaderProps) {
  ...
  // Use brand theme only if header is in hero section, otherwise use default
  const currentTheme = isInHeroSection 
    ? (brandThemes[currentBrand] || brandThemes.default)
    : brandThemes.default;
  ...
  // Social media links - in order: Behance, Instagram, Pinterest, WhatsApp, Threads, Facebook
  const socialLinks = [
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Check if header is in hero section by finding the hero element
      const heroSection = document.querySelector('[data-cursor-brand]');
      if (heroSection) {
        const heroRect = heroSection.getBoundingClientRect();
        // Check if the header (top of viewport) is still within hero bounds
        // Header is at top of viewport (scrollY = 0 in viewport coords)
      ...
      // More sensitive scroll direction detection with velocity threshold
      ...
      // Add a small delay to prevent flickering during rapid scroll changes
      scrollTimeout = setTimeout(() => {
        // This ensures smooth transitions don't get interrupted
      }, 150);
    };
  ...
  // Enhanced Framer Motion animation variants with longer, smoother durations
  const menuVariants = {
    closed: {
      opacity: 0,
      transition: {
  ...
  // Smooth header visibility with longer fade durations
  ...
  return (
    ...
    >
      <div className="container max-w-none mx-auto px-8 py-8 flex items-center justify-between">
        ...
        >
          {/* Only render animations after mount to prevent both icons showing */}
          {isMounted && (
            <>
              <motion.div 
